...
The investigation traced a simulated PowerShell payload from creation through staging and execution on a Windows 11 endpoint. File metadata and SHA256 hashes were used to verify the staged copy, while execution artifacts confirmed successful execution.

Wazuh FIM and Sysmon telemetry were reviewed to support the timeline and identify related activity. The evidence confirmed the controlled payload workflow, while some security events required additional correlation before they could be linked to the payload.
